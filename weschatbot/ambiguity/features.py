...
def _compute_cluster_sizes(labels):
    return {int(x): int(c) for x, c in zip(*np.unique(labels[labels != -1], return_counts=True))}
...
class KMeansFeatures:
    ...
    def compute(vectors, labels, scores, distances, centers, k, question_id, algorithm, n_samples):
        ...
        features = {
            ...
            'cluster_gini': float((2 * np.sum((np.arange(1, len(size_values) + 1) * np.sort(size_values)))) / (len(size_values) * np.cumsum(np.sort(size_values))[-1]) - (len(size_values) + 1) / len(size_values)),  # noqa
            ...
            'dominance_gap': float(sorted_sizes[0] / n_samples) - (
                float(sorted_sizes[1] / n_samples) if len(sorted_sizes) > 1 else 0),
        }
        ...
        if len(centers) >= 2:
            ...
            features.update(
                {'inter_cluster_dist_mean': 0.0, 'inter_cluster_dist_min': 0.0, 'inter_cluster_dist_max': 0.0,
...
class HDBSCANFeatures:
    ...
    def compute(vectors, labels, scores, probabilities, question_id, algorithm, params):
        ...
        features.update({
            ...
            'normalized_entropy': float(entropy(cluster_probs) / np.log(n_clusters)) if len(
                cluster_probs) > 0 and n_clusters > 1 else 0.0,
            'n_major_clusters': major_clusters,
            'has_multiple_major_clusters': int(major_clusters >= 2),
        })
        ...
        cluster_low_ratios = [(probabilities[labels == cid] < low).sum() / (labels == cid).sum() for cid in
                              unique_clusters]
        ...
        if n_noise > 0:
            ...
            features.update({
                ...
                'noise_vs_cluster_prob_diff': float(noise_probs.mean() - non_noise_probs.mean()) if len(
                    non_noise_probs) > 0 else 0,
                'noise_vs_cluster_score_diff': float(noise_scores.mean() - non_noise_scores.mean()) if len(
                    non_noise_scores) > 0 else 0,
            })
        else:
        ...
        features['score_variation_between_clusters'] = _safe_div(np.std(cluster_score_means),
                                                                 np.mean(cluster_score_means))
        features['probability_score_corr'] = float(np.corrcoef(probabilities, scores)[0, 1]) if len(
