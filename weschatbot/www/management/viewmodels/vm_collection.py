...
class ViewModelCollection(ViewModel, LoggingMixin):
    ...
    def add_item_post(self, session=None):
        ...
        try:
            ...
            res = CollectionService.create_collection(collection_name=name, milvus_host=config["milvus"]["host"],
                                                      milvus_port=config.getint("milvus", "port"))
            if res:
    ...
    def detail_item(self, item_id, session=None):
        try:
            ...
            flash(f"Error getting detail of item {item_id}", "danger")
            return f"{e}", 500

    @provide_session
    def delete_item_post(self, item, session=None):
        try:
            self.collection_service.delete_collection(collection_id=int(item.id))
            flash("Successfully deleted the item", "success")
            return redirect(self.list_view_model.search_url_func()), 302
        except Exception as e:
            self.log.exception(e)
            flash(f"Error delete item: {item.id}", "danger")
            return redirect(self.list_view_model.search_url_func()), 302
    ...
    def delete_entities(self, session=None):
        try:
            ...
            self.collection_service.delete_entities(collection_id=collection_id, row_id=int(row_id), session=session)
        except Exception as e:
            return jsonify({"status": "error", "message": str(e)}), 500
        return jsonify({"status": "success"}), 200
